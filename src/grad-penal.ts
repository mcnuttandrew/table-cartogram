...
import {derivPenalty, computeMinDistPoint, computeEdges} from './objective-function';

function gradOrderPenalty(props: PenaltyProps): {x: number; y: number} {
  ...
  return evalTarget.reduce(
    (grad, {val, dim, lessThan}) => {
...
function gradOverlapPenalty(props: PenaltyProps): {x: number; y: number} {
  const {cell, newTable, inCorner, inFirstRow, inLastRow, i, j, inLeftColumn, inRightColumn} = props;
  let neighbors = [] as {x: number; y: number}[];
  if (inCorner) {
    // no penaltys for corners, they are not manipualted
  } else if (inFirstRow) {
    neighbors = [
  ...
  if (neighbors.length && !pointInPolygon([cell.x, cell.y], points)) {
    const {minPoint, minDist} = computeMinDistPoint(points as ArrPos[], cell);
...
export function gradBuildPenalties(
  newTable: PositionTable,
  dims: Dimensions,
  optimizationParams: OptimizationParams,
): {x: number; y: number}[][] {
  const tableGradient = [];
  const {borderPenalty, orderPenalty, overlapPenalty} = optimizationParams;
...
function errorGrad(newTable: PositionTable, targetTable: DataTable): {x: number; y: number}[][] {
  ...
  for (let i = 0; i < rects.length; i++) {
    for (let j = 0; j < rects[0].length; j++) {
      ...
      offsets.forEach((offset, index) => {
        ...
        const {inFirstRow, inLeftColumn, inRightColumn, inLastRow, inCorner} = computeEdges(
          newTable,
          idx,
          jdx,
        );
...
export function buildErrorGradient(
  vector: Vector,
  targetTable: DataTable,
  dims: Dimensions,
  searchIndices: Vector,
  optimizationParams: OptimizationParams,
): Vector {
