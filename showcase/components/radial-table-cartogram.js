...
import {XYPlot, PolygonSeries, ArcSeries} from 'react-vis';

import {colorCell} from '../colors';

export default function radilaCartogramPlot(props, setX, xFlip, yFlip) {
  const {data, fillMode, height = 600, width = 600, rectStyle = {}} = props;
  ...
  const {xMax, yMax} = data.reduce(
    (acc, row) => {
      return row.vertices.reduce(
        (mem, {x, y}) => ({
  ...
  return (
    <XYPlot margin={0} width={width} height={height} xDomain={[-2, 2]} yDomain={[-2, 2]}>
      {showArcs && (
        <ArcSeries
          colorType={'literal'}
          center={{x: 0, y: 0}}
      ...
      {showBoxes &&
        data.map((cell, index) => {
          return (
            <PolygonSeries
              key={`quad-${index}`}
              data={cell.vertices.map(({x, y}) => ({
                x: y * Math.cos(x * Math.PI * 2),
                y: y * Math.sin(x * Math.PI * 2),
              }))}
              style={{
                strokeWidth: 1,
