...
export default class HourCalendar extends React.Component {
  ...
  componentDidMount() {
    ...
    const houredData = Object.entries(
      data.reduce((acc, row) => {
      ...
      .map((d) =>
        d[1]
          .map((row) => ({
            ...row,
            Temperature: celius ? farenheitToCelius(row.Temperature) : row.Temperature,
          }))
    ...
    Promise.resolve().then(() => {
      ...
      const yLabels = gons
        .filter((d) => d.data.Day === '16')
        .map((cell, index) => {
          const hour = Number(cell.data.Hour);
          return hour % 12 ? `${hour % 12} ${hour >= 12 ? 'PM' : 'AM'}` : hour >= 12 ? 'NOON' : 'MIDNIGHT';
        });
      this.setState({
        gons,
        xLabels,
