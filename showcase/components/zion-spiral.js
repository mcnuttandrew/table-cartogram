...
// WORK ON THIS
const rScale = scaleLinear().domain([
  ZION_VISITORS_WITH_ANNOTATION_DOMAIN.min,
  ZION_VISITORS_WITH_ANNOTATION_DOMAIN.max
]).range([0, 1]);
...
export default class ZionExperiment extends React.Component {
  render() {
    return (
      ...
      {
        <XYPlot
        yDomain={[0, ZION_VISITORS_WITH_ANNOTATION_DOMAIN.max]}
        height={500}
        width={500}>
          <YAxis tickFormat={d => `${Math.floor(d / 1000)}k`}/>
          <XAxis
            tickFormat={d => MONTHS[d]}
            tickValues={[...new Array(12)].map((_, idx) => idx)}/>
          {
