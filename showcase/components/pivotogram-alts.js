...
const setColr = (value) => interpolateReds(1 - Math.sqrt(1 - (value - 0) / (705000 - 0)));

const lakeDomain = AlongTheLake.reduce(
  (acc, row) => {
    row.forEach(({value}) => {
...
const data = {
  children: transposeMatrix(AlongTheLake).map((row) => {
    return {
...
function LineChart() {
  return (
    ...
    >
      ...
      />
      <LabelSeries
        data={transposeMatrix(AlongTheLake).map((row, key) => {
