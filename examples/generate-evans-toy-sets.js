...
function convertTableToEvansFormat(table) {
  ...
  const namedLayout = [...new Array(dataTable.length)].map((_, idx) => {
    return [...new Array(dataTable[0].length)].map((__, jdx) => `x${jdx}y${idx}`).join(' ');
  }).join('\n');

  const valueDomain = table.data.reduce((acc, row) => {
  ...
  const nameValues = [...new Array(dataTable.length)].map((_, idx) => {
    return [...new Array(dataTable[0].length)].map((__, jdx) => {
